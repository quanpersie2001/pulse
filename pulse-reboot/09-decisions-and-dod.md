...
- [ ] Story không đóng nếu required behavioral receipt thiếu/invalid/fail/flaky/inconclusive hoặc coverage gap chưa disposition. **Partial:** `story_close` receipt đã bind Story, full applicable baseline và frozen source; resolver này đã gate các child Ticket `covered_by_story_close`. Story node lifecycle close và các matrix/waiver gates còn thiếu.
