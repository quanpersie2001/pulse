...
- [x] Generated docs freshness có deterministic contract/check. Phase 3 Slice 2 chạy exact declared `freshness_check` bằng argv/no-shell, aggregate local link và `_index.md` projection findings, và fail closed trước execution khi registry invalid. Slice 3 ghi source/content-bound documentation receipt, bind exact `verification_profile` và reject snapshot/profile drift. Slice 4 consume current gate-eligible receipt cho exact required-document close coverage; profile-specific review acquisition và documentation defer/promotion vẫn deferred.
...
- [x] Story QA baseline chạy được ít nhất qua một deterministic browser/Playwright executor và một structured non-browser API/CLI executor. Repository-owned structured executor và full Story qualification đã chạy qua daemon; dedicated `playwright-service` target fixture đã launch Chromium thật, chạy deterministic rendered-page assertions, collect console/network observations, ingest real trace ZIP và replay Story close. Browser acceptance là explicit ignored command vì phải cài pinned Playwright/browser, không phải fake shell output hoặc silent skip.
- [x] QA environment start/healthcheck/fixture reset/cleanup và source-to-build identity tham gia receipt validity. Payload version 4 bind `source_commit -> build_id -> deployment_id/base_url` xuyên suốt lifecycle, browser report, trace receipt và current Core close eligibility; mismatch fail closed, cleanup được chứng minh, còn payload version 1/2/3 giữ historical integrity. Generic cloud deployment provider không thuộc criterion local-first này.
- [x] Story không đóng nếu required behavioral receipt thiếu/invalid/fail/flaky/inconclusive hoặc coverage gap chưa disposition. Required environment/platform matrix resolve exact cases per entry; specialized Story close yêu cầu một current passed qualification head cho mọi entry, reject unlinked attempts và commit outcome atomically.
- [x] Retry giữ failed attempts; waiver/non-applicability có rationale, authority và audit. Retry receipts nối predecessor immutable; pass-after-failure vẫn flaky trừ `qa.flaky.waive` bind current policy revision/fingerprint. Case `not_applicable` cần matching rationale và approval có `qa.non_applicable.approve`.
