...
- [ ] Story không đóng nếu required behavioral receipt thiếu/invalid/fail/flaky/inconclusive hoặc coverage gap chưa disposition. **Partial:** `story_close` receipt đã bind Story, full applicable baseline và frozen source; resolver gate child Ticket `covered_by_story_close`, và specialized Story lifecycle close yêu cầu terminal descendants, no open blocker, independent full-current qualification trước atomic `done`. Required matrix và waiver policy vẫn còn thiếu.
