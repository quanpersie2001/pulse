...
7. Verification maps every acceptance item to passing checks/evidence; Core may close a low-risk Ticket whose documentation posture is `none` or whose exact required documents have current payload-v2 validation proof. QA posture may be `none`, satisfied by a required checkpoint, or covered by a current full Story qualification.
