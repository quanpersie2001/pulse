...
7. Verification maps every acceptance item to passing checks/evidence; Core may close a low-risk Ticket whose documentation posture is `none` or whose exact required documents have current documentation validation proof. QA posture may be `none`, satisfied by a required checkpoint, or covered by a current full Story qualification.
8. Required documentation closes only with a current gate-eligible `documentation_validation` receipt on the verified source and exact required document revisions/content/profile. Required Ticket QA closes only with a current passed `qa_checkpoint` receipt whose `qa_scope` is `ticket_checkpoint` and which covers the exact affected Story cases. A QA-deferred Ticket requires a current passed receipt whose `qa_scope` is `story_close`, covering the full applicable Story baseline on the same source. Documentation `deferred`, profile-specific semantic review acquisition and medium-or-higher Ticket risk remain fail-closed until their dedicated assurance resolvers are installed.
