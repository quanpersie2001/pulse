...
Intake creates or matches the owning EPIC/STORY boundary and writes `intake.md`. Planning queries that boundary through `{{pulse_command}} workgraph ... --json`, enriches the existing epic/story `README.md` content, then materializes approved current-slice TASK/BUG metadata through `{{pulse_command}} workgraph` commands. Agents and humans must treat `.pulse/workgraph/items.jsonl` as database-like storage behind the CLI: do not read or hand-edit it during planning, and do not create duplicate EPIC/STORY items when intake already established the boundary.
