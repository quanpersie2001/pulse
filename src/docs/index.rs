...
/// Write the tracked navigation projections for the current registry.
///
/// `build_index` writes these as part of a full index build, which also builds
/// the disposable docs-search cache. Enrolment needs only the tracked half:
/// `pulse init` registers `DOC-GLOSSARY`, and `docs validate` then requires the
/// projection that registration implies, so a repository that never ran this
/// is invalid from the moment it is initialised.
///
/// Projections are derived, so a caller that reports enrolment has to tell a
/// first write apart from a re-run that produced identical bytes.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct NavigationProjectionWrite {
    /// Repository-relative paths that did not exist, or whose bytes changed.
    pub changed: Vec<String>,
    /// Repository-relative paths already holding exactly these bytes.
    pub unchanged: Vec<String>,
}

/// # Errors
///
/// Returns a typed error when the registry cannot be read, a projection path
/// is unsafe, an existing file at a projection path was not generated by Pulse,
/// or the write fails.
pub fn write_navigation_projections(repo_root: &Path) -> PulseResult<NavigationProjectionWrite> {
    let registry: DocsRegistry = crate::storage::read_json(&registry_path(repo_root))?;
    let mut result = NavigationProjectionWrite::default();
    for target in projection_targets(&registry) {
        let rendered = match &target.area {
        ...
        if identical {
            ...
            result.changed.push(target.path);
        }
    }
    write_projections(repo_root, &registry)?;
    Ok(result)
