...
fn validate_decision_acceptance_payload(
    receipt: &ReceiptEnvelope,
...
fn validate_actor(actor: &ActorRef) -> Result<()> {
...
fn validate_unique_by<'a>(code: &'static str, values: impl Iterator<Item = &'a str>) -> Result<()> {
    let mut seen = BTreeSet::new();
    for value in values {
...
fn validate_id_prefix(id: &str, prefix: &str, code: &'static str) -> Result<()> {
...
fn validate_work_id_kind(id: &str, prefix: &str, code: &'static str) -> Result<()> {
...
fn validate_content_hash(hash: &str, code: &'static str) -> Result<()> {
...
fn validate_non_empty(value: &str, code: &'static str, message: &'static str) -> Result<()> {
