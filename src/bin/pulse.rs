            ...
            let knowledge = {
                #[cfg(debug_assertions)]
                {
                    match store.failpoint() {
                        Some(failpoint) => {
                            KnowledgeStore::with_failpoint(store.repo_root(), failpoint)
                        }
            ...
            match command {
                ...
                KnowledgeCommand::Edit {
                    ...
                    let bytes = std::fs::read(&patch)
                        .map_err(|error| PulseError::io(patch.clone(), error))?;
