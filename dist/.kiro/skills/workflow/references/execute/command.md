...
- `.pulse/workgraph/items.jsonl` is mutated only through `node .kiro/skills/workflow/scripts/pulse.mjs workgraph ...`.
...
```bash
node .kiro/skills/workflow/scripts/pulse.mjs workgraph show --repo-root <repo> <item-id> --json
```
...
```bash
node .kiro/skills/workflow/scripts/pulse.mjs reservation reserve \
  --repo-root <repo> \
...
```bash
node .kiro/skills/workflow/scripts/pulse.mjs workgraph update --repo-root <repo> <item-id> --status IN_PROGRESS --json
```
...
```bash
node .kiro/skills/workflow/scripts/pulse.mjs workgraph close --repo-root <repo> <item-id> --json
```
...
2. `node .kiro/skills/workflow/scripts/pulse.mjs status --repo-root <repo> --json`
3. `.pulse/runtime/state.json`
4. current item metadata via `node .kiro/skills/workflow/scripts/pulse.mjs workgraph show --repo-root <repo> <item-id> --json`
...
8. active reservations:
   ```bash
   node .kiro/skills/workflow/scripts/pulse.mjs reservation list --repo-root <repo> --active-only --json
   ```
