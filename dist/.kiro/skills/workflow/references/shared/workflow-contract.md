...
Use `node .kiro/skills/workflow/scripts/pulse.mjs` to inspect readiness and coordinate runtime reservations once the runtime exists.

Examples:

- `pulse:workflow plan` decides the shape of work.
- `node .kiro/skills/workflow/scripts/pulse.mjs ready --repo-root <repo> --json` surfaces executable work.
