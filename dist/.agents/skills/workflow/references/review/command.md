...
When a finding requires implementation work, create or propose a workgraph `BUG` or `TASK` under the active story using `node .agents/skills/workflow/scripts/pulse.mjs workgraph create ... --json`, but only when the user/project policy permits review to materialize fix work.
