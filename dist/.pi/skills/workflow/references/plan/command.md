...
- active epic/story workgraph state queried through `node .pi/skills/workflow/scripts/pulse.mjs workgraph ... --json`
...
After explicit Gate 2 approval, materialize only the approved TASK/BUG rows and edge rows recorded in `plan.md`. Use the rendered `node .pi/skills/workflow/scripts/pulse.mjs` value from the installed workflow skill; do not call `scripts/pulse.mjs` by a guessed filesystem path.
...
```bash
node .pi/skills/workflow/scripts/pulse.mjs workgraph create \
  --repo-root <repo> \
  --kind TASK \
  --parent <story-id> \
...
```bash
node .pi/skills/workflow/scripts/pulse.mjs workgraph dep add \
  --repo-root <repo> \
  <item-id> \
  <depends-on-item-id> \
  --json
...
```bash
node .pi/skills/workflow/scripts/pulse.mjs workgraph dep add --repo-root <repo> T-13 T-12 --json
```
...
```bash
node .pi/skills/workflow/scripts/pulse.mjs workgraph link add \
  --repo-root <repo> \
  <item-id> \
  <linked-item-id> \
  --json
...
- item IDs and content paths are not hand-authored in `plan.md`; use placeholders until `node .pi/skills/workflow/scripts/pulse.mjs workgraph create ... --json` returns canonical values
- before Gate 2 approval, record the intended TASK/BUG items and edge posture only
- after Gate 2 approval, create items with `node .pi/skills/workflow/scripts/pulse.mjs workgraph create ... --json`, then add approved edges with `node .pi/skills/workflow/scripts/pulse.mjs workgraph dep add ... --json` and `node .pi/skills/workflow/scripts/pulse.mjs workgraph link add ... --json`
- after materialization, run or request `node .pi/skills/workflow/scripts/pulse.mjs workgraph doctor --repo-root <repo> --json`
...
3. Query active epic/story through `node .pi/skills/workflow/scripts/pulse.mjs workgraph ... --json`.
4. Confirm:
...
3. create approved TASK/BUG items through `node .pi/skills/workflow/scripts/pulse.mjs workgraph create ... --json`
4. write task README content at each returned `content_path` using [task.readme.md](./task.readme.md)
5. add approved dependency/link edges through `node .pi/skills/workflow/scripts/pulse.mjs workgraph dep/link ... --json`
6. run or request `node .pi/skills/workflow/scripts/pulse.mjs workgraph doctor --repo-root <repo> --json`
...
- approved TASK/BUG materialization applied only through `node .pi/skills/workflow/scripts/pulse.mjs workgraph`
