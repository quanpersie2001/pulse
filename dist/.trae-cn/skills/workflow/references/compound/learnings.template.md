...
tags: [tag1, tag2, tag3]                   # domains covered (e.g., auth, testing, workgraph)
...
**Propagation:** global-critical | work-item-local | planner-only
**Embed-in-work-item-when:** <one sentence for future planners>
**Work item hint:** <1-2 sentence summary safe to copy into work-item context if relevant>
```
...
**Embed-in-work-item-when:** The work item touches token refresh, session renewal, or token rotation behavior
**Work item hint:** Token rotation is concurrency-sensitive. Use an atomic database operation and include a parallel-request verification path.
...
**Propagation:** work-item-local
**Embed-in-work-item-when:** A future work item adds or changes frontend auth state handling
**Work item hint:** Reuse `useAuthToken()` instead of writing parallel token retrieval logic.
```
...
- Primary topic = most important domain (`auth`, `database`, `workgraph`, `api`, `testing`)
...
- Examples:
  ...
  - `20260320-workgraph-scope-isolation.md`
