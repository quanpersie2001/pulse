# `pulse:workflow brainstorm` Spec Reviewer Prompt

Use this when dispatching a spec self-review after brainstorming has produced a written design artifact.

**Purpose:** Verify the design is complete, internally consistent, and ready for `pulse:workflow explore` to lock repo-grounded decisions without guesswork.
...
Task tool (general-purpose):
  ...
  prompt: |
    You are reviewing a `pulse:workflow brainstorm` output. Verify that the chosen direction is
