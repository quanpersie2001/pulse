...
Do not pre-assign permanent tracks. Workers self-route from live `node .trae/skills/workflow/scripts/pulse.mjs ready --repo-root <repo> --json` output after startup checks.
...
Stay in tending mode while any worker is active, blocked, expected to report, or while `node .trae/skills/workflow/scripts/pulse.mjs ready --repo-root <repo> --json` still returns executable work.
...
4. Re-check `node .trae/skills/workflow/scripts/pulse.mjs ready --repo-root <repo> --json` and active workgraph posture after significant transitions.
