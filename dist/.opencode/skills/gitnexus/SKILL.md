...
name: gitnexus
description: >-
  Use when Pulse planning or discovery needs graph-based architecture context,
  execution-flow tracing, symbol relationships, or blast-radius analysis beyond
  plain file search.
metadata:
  version: '1.0'
  ecosystem: pulse
...
```bash
node .opencode/skills/pulse:workflow/scripts/pulse.mjs status --repo-root <repo> --json
```
