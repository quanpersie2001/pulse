...
Do not pre-assign permanent tracks. Workers self-route from live `node .github/skills/pulse:workflow/scripts/pulse.mjs ready --repo-root <repo> --json` output after startup checks.
...
Stay in tending mode while any worker is active, blocked, expected to report, or while `node .github/skills/pulse:workflow/scripts/pulse.mjs ready --repo-root <repo> --json` still returns executable work.
...
4. Re-check `node .github/skills/pulse:workflow/scripts/pulse.mjs ready --repo-root <repo> --json` and active workgraph posture after significant transitions.
5. Refresh reservation posture when conflicts or stalled workers appear:

```bash
node .github/skills/pulse:workflow/scripts/pulse.mjs reservation list --repo-root <repo> --active-only --json
```
