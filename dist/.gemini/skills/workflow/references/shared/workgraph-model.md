...
- `plan` should produce lowercase `plan.md` shapes and materialize approved current-slice items through `node .gemini/skills/workflow/scripts/pulse.mjs workgraph`, never by hand-editing `.pulse/workgraph/items.jsonl`.
- `validate` should test whether the Gate 2-approved `plan.md` and materialized TASK/BUG items are ready for execution, using `node .gemini/skills/workflow/scripts/pulse.mjs workgraph` output rather than hand-editing `.pulse/workgraph/items.jsonl`.
