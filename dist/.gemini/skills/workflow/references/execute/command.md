...
- `.pulse/workgraph/items.jsonl` is mutated only through `node .gemini/skills/workflow/scripts/pulse.mjs workgraph ...`.
...
```bash
node .gemini/skills/workflow/scripts/pulse.mjs workgraph show --repo-root <repo> <item-id> --json
```
...
```bash
node .gemini/skills/workflow/scripts/pulse.mjs reservation reserve \
  --repo-root <repo> \
...
```bash
node .gemini/skills/workflow/scripts/pulse.mjs workgraph update --repo-root <repo> <item-id> --status IN_PROGRESS --json
```
...
```bash
node .gemini/skills/workflow/scripts/pulse.mjs workgraph close --repo-root <repo> <item-id> --json
```
...
2. `node .gemini/skills/workflow/scripts/pulse.mjs status --repo-root <repo> --json`
3. `.pulse/runtime/state.json`
4. current item metadata via `node .gemini/skills/workflow/scripts/pulse.mjs workgraph show --repo-root <repo> <item-id> --json`
...
8. active reservations:
   ```bash
   node .gemini/skills/workflow/scripts/pulse.mjs reservation list --repo-root <repo> --active-only --json
   ```
