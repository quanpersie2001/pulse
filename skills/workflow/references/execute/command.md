...
- `.pulse/workgraph/items.jsonl` is mutated only through `{{pulse_command}} workgraph ...`.
...
```bash
{{pulse_command}} workgraph show --repo-root <repo> <item-id> --json
```
...
```bash
{{pulse_command}} reservation reserve \
  --repo-root <repo> \
...
```bash
{{pulse_command}} workgraph update --repo-root <repo> <item-id> --status IN_PROGRESS --json
```
...
```bash
{{pulse_command}} workgraph close --repo-root <repo> <item-id> --json
```
...
2. `{{pulse_command}} status --repo-root <repo> --json`
3. `.pulse/runtime/state.json`
4. current item metadata via `{{pulse_command}} workgraph show --repo-root <repo> <item-id> --json`
