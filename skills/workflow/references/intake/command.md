...
Immediately before presenting this structural-mutation hard gate, run or verify a fresh `{{pulse_command}} status --repo-root <repo> --json` posture proof. This is separate from the initial posture gate in Phase 1. If proof is stale, missing, or contradicted by `.pulse/runtime`, `.pulse/workgraph`, or `works/` changes during classification, do not present the creation/update hard gate; route back to `pulse:workflow use` or ask the operator to rerun status first.
...
1. Run `{{pulse_command}} status --repo-root <repo> --json` again.
