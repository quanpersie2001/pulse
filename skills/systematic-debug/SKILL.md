...
description: Use when the user asks for root-cause-first debugging of bugs, test failures, flaky behavior, build issues, or integration defects with tracked verification and regression lock-down.
metadata:
  version: '1.1'
  dependencies: []
---
...
Can be invoked during `pulse:workflow execute` for beads that involve bug fixes or defect resolution. When used within Pulse, feed the fix tracker output and regression test results into the bead's verification evidence and `learning_refs` for `pulse:workflow compound`.
