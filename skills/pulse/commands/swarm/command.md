...
Run `/pulse swarm` only when all are true:
...
If `single-worker` is recommended, route to `/pulse execute`.
...
Spawn bounded workers and require each to run `/pulse execute`.
...
- final slice complete -> recommend `/pulse review`
- more slices remain -> recommend `/pulse plan`
