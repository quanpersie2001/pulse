...
If material mismatch exists, block and route to `/pulse plan` or `/pulse execute` as appropriate.
...
- exact next command recommendation:
  - `/pulse execute` for fixes
  - `/pulse plan` for scope/shape repair
  - `/pulse compound` after clean pass
