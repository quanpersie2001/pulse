...
- worker execution under `/pulse swarm`
...
- if invoked by `/pulse swarm`, run in worker mode
...
1. `AGENTS.md`
2. if present: `node .pulse/scripts/pulse_status.mjs --json`
...
```bash
node .pulse/scripts/pulse_reservations.mjs reserve --agent <runtime_identity> --item <item-id> --path "src/foo.ts" --path "src/bar.ts" --json
```
...
Write handoff using command-local contract at `skills/pulse/commands/execute/references/handoff-contract.md`:
...
5. active reservations: `node .pulse/scripts/pulse_reservations.mjs list --active-only --json`
...
| Reserve scope | `node .pulse/scripts/pulse_reservations.mjs reserve --agent <runtime_identity> --item <item-id> --path "..." --json` |
| Release scope | `node .pulse/scripts/pulse_reservations.mjs release --agent <runtime_identity> --json` |
| List active reservations | `node .pulse/scripts/pulse_reservations.mjs list --active-only --json` |
...
## Inputs from `/pulse swarm` (worker mode)
