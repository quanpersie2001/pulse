...
<HARD-GATE>
Do NOT invoke `/pulse explore`, `/pulse plan`, or any implementation command, write any code, create work items, or take implementation action until a design has been presented AND the user has approved it.
...
| Handoff | Update `.pulse/runtime/STATE.md` and recommend `/pulse explore` as the next manual step | Clean pipeline transition |
...
10. **Handoff to `/pulse explore`** — recommend `/pulse explore` as the next manual step to lock implementation decisions
...
digraph brainstorming {
    ...
    "Recommend /pulse explore handoff" [shape=doublecircle];
    ...
    "User reviews spec?" -> "Recommend /pulse explore handoff" [label="approved"];
}
```

**The terminal state is an approved design artifact under `works/` plus `.pulse/runtime/STATE.md` update, with a recommendation to run `/pulse explore` next.** Do NOT invoke planning, validating, or any execution command by default. After brainstorming, the only valid next step is `/pulse explore`.
...
1. Update `.pulse/runtime/STATE.md`:
   ...
   Next: invoke /pulse explore to lock implementation decisions
   ```

2. Present to the user:
   > "Spec approved. Next step: run `/pulse explore` to extract implementation decisions — gray areas, scope boundaries, and locked choices — before planning begins."

3. Optional continue-now path:
   - Only if the user explicitly asks to continue now, invoke `/pulse explore` in the same session.
   - Otherwise stop after the spec approval, `.pulse/runtime/STATE.md` update, and recommendation above.

<HARD-GATE>
Do NOT invoke `/pulse plan`, create work items, or write any code.
The terminal state of this command is a written, approved spec and state update.
The only valid next step is `/pulse explore`.
</HARD-GATE>
...
- **Do not scaffold project docs here** — brainstorming can consume existing docs, but durable project-doc scaffolding belongs to `/pulse explore` when needed.

---

## What this command does NOT do

These are `/pulse explore` responsibilities, not brainstorming responsibilities:
...
The spec can be three sentences. But it MUST exist so `/pulse explore` has a stable target.
