...
function buildPulseBootstrapBlock() {
  ...
  return [
    ...
    "You have Pulse.",
    "",
    "Below is the full content of your `/pulse onboard` bootstrap skill. Use it to route safely before loading downstream Pulse skills:",
    "",
    skillText,
    "</EXTREMELY_IMPORTANT>",
  ].join("\n");
...
export async function collectPulseSessionStartNotes(repoRoot, options = {}) {
  ...
  if (fs.existsSync(onboardingPath)) {
    ...
    notes.push("Onboarding readiness has not been established for this repo. Run /pulse onboard before continuing.");
  }
