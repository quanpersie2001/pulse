...
| `execute` | Execution | a validated work item should be implemented and evidenced | [commands/execute/command.md](commands/execute/command.md) | `review`, `rescue`, `systematic-debug` |
...
| `compound` | Learning | the completed cycle should produce reusable learnings | [commands/compound/command.md](commands/compound/command.md) | `note-distill`, `plan` |
| `rescue` | Intervention | execution is stuck because the current shape or architecture is wrong | [commands/rescue/command.md](commands/rescue/command.md) | `plan`, `validate`, `execute` |
| `systematic-debug` | Intervention | a bug needs disciplined investigation before a fix is chosen | [commands/systematic-debug/command.md](commands/systematic-debug/command.md) | `execute`, `review`, `rescue` |
| `note` | Notes | a tactical decision, breadcrumb, or partial insight should be captured quickly | [commands/note/command.md](commands/note/command.md) | `note-distill`, active workflow command |
| `note-distill` | Notes | multiple raw notes should become synthesized guidance | [commands/note-distill/command.md](commands/note-distill/command.md) | `compound`, `plan`, `review` |
...
| --- | --- |
| `/pulse` | choose the workflow move, load command guidance, preserve gate discipline |
