...
- `src/graph/` is the work-graph domain, layered strictly bottom-up:
  ...
  - `graph::store` — persistence: sharded JSON node/edge files, manifest,
    ...
    mutation seams, supersession. `JsonGraphStore` is the public facade.
...
|---|---|---|
| Runner (`pulse run <role>`) | §5.3 | lease machinery exists in `kernel::reservation`; command-execution contracts (`qa::executor`) are the seed. Spawning, packet delivery and recovery saga were archived to `design/archive/daemon-assignment-saga.rs`. |
